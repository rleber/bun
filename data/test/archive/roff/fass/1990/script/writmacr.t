...
.el skip_all_macros
.an skip_all_macros 1
.ze >>> Writers' Macros. Use correct o= value for slist, otherwise klist <<<
...
.ta 4 L C R 57
	...
	===  Music Cues: ^^(music_count)	Light Cues: ^^(light_count)	Sound Cues: ^^(sound_count)  ===
	===  Songs: ^^(song_count)    Special Effects: ^^(sfx_count)		Props: ^^(prop_count)  ===
	======================================================
.mg
   ===                                                ===
.in 8
...
.en es_endif
.ze >>> Pages=^^(save_page) Speeches=^^(speech_count) Characters=^^(chars_used) Music=^^(music_count) Sound=^^(sound_count) Light=^^(light_count) <<<
.ze >>> Songs=^^(song_count) Special Effects=^^(sfx_count) Props=^^(prop_count) <<<
.en es
.zz -- end a scene
.at fini
