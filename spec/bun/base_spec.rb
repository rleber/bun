...
UNPACK_PATTERNS = {
  :unpack_time=>/:unpack_time:?\s+\d{4}-\d\d-\d\d \d\d:\d\d:\d\d(?:\.\d{9})? [-+]\d\d:?\d\d\s*\n?/,
...
DECODE_PATTERNS = {
  :unpack_time=>/:unpack_time:?\s+\d{4}-\d\d-\d\d \d\d:\d\d:\d\d(?:\.\d{9})? [-+]\d\d:?\d\d\n?/,
  :unpacked_by=>/:unpacked_by:?\s+"?Bun version \d+\.\d+\.\d+\s+\[.*?\]"?\n?/, 
  :decode_time=>/:decode_time:?\s+\d{4}-\d\d-\d\d \d\d:\d\d:\d\d(?:\.\d{9})? [-+]\d\d:?\d\d\n?/,
