...
option "shards",   :aliases=>"-S", :type=>'string',  :default=>'.*',              :desc=>"Show only files that match this Ruby Regexp, e.g. 'f.*oo\\.rb$'"
...
def ls(file_name)
  ...
  file_pattern = get_regexp(options[:shards])
  stop "!Invalid --shards pattern. Should be a valid Ruby regular expression (except for the delimiters)" unless file_pattern
