...
#   ['project']
#
#   A thorough description of the algorithm is available at http://tug.org/docs/liang/liang-thesis.pdf
#   
#   Original Python code by Ned Batchelder, July 2007. 
#   http://nedbatchelder.com/code/modules/hyphenate.py
#   Python code is in the public domain.
#
#   Translated to Ruby by Richard LeBer, 2014

VERSION = '1.0.20070709'

module Hyphenator
  class KnuthLiang < Base
    VERSION = '1.0.20070709'

    attr_reader :patterns
    attr_reader :exceptions
    attr_reader :tree
    attr_reader :longest_key # Used by Hyphenator::Base

    def initialize(patterns=PATTERNS, exceptions=EXCEPTIONS)
      @longest_key = 0
    ...
    def patterns=(patterns)
      ...
      patterns.split.each do |pattern|
        _insert_pattern(pattern)
      end
    end

    def _insert_pattern(pattern)
      # Convert the a pattern like 'a1bc3d4' into a string of chars 'abcd'
      # and a list of points [ 1, 0, 3, 4 ].
      chars = pattern.gsub(/[0-9]/,'').split(//)
      points = pattern.split(/[.a-z]/).map {|d| d.to_i}
      @longest_key = chars.size if chars.size > @longest_key
    ...
    private :_insert_pattern
    
    # Convert the hyphenated exceptions into a point hash for use later.
    def exceptions=(exceptions)
      @exceptions = {}
      exceptions.split.each do |ex|
        word = ex.gsub('-', '')
        @longest_key = word.size if word.size > @longest_key
        points = [0] + ex.split(/[a-z]/).map{|ch| ch=='-' ? 1 : 0}
    ...
    def hyphenate(word)
      ...
      if (e=@exceptions[word])
        points = e
      else
        work = '.' + word + '.'
        points = [0] * (work.size+1)
        (0...work.size).each do |i|
          t = @tree
          work[i..-1].split(//).each do |c|
            if t[c]
              t = t[c]
              if t[nil]
                p = t[nil]
                (0...p.size).each do |j|
                  points[i+j] = [points[i+j], p[j]].max
                end
              end
            else
              break
      ...
      pieces = ['']
      word.split(//).zip(points[2..-1]).each do |c, p|
        pieces[-1] += c
        unless p.nil? || p % 2 == 0
          pieces << ''
        end
    ...
    PATTERNS = STANDARD_PATTERNS + ' ' + EXTRA_PATTERNS
